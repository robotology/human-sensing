...
docker run -it --rm --privileged --gpus=all --network host --pid host\
  ...
  -v ${XAUTHORITY}:/root/.Xauthority -v ~/.config/yarp:/root/.config/yarp \
  -e DISPLAY=$DISPLAY -e QT_X11_NO_MITSHM=1 -v /etc/hosts:/etc/hosts \
  -v $DATA_DIR_PATH:/${DATA_DIR} --name $CONTAINER_NAME $IMG $CMD